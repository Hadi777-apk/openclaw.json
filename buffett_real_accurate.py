...
"""
巴菲特风格选股 - 手动计算毛利率（准确版）
毛利率 = (total_revenue - total_cogs) / total_revenue * 100
"""
...
for ts_code, name in focus_stocks:
    ...
    try:
        ...
        if all_pass:
            ...
            if not margin_pass:
                if latest_gross_margin is not None:
                    reasons.append(f"毛利率{latest_gross_margin:.1f}%")
                else:
                    reasons.append("毛利率 N/A")
            if not debt_pass:
                if debt_to_assets is not None:
                    reasons.append(f"负债率{debt_to_assets:.1f}%")
                else:
                    reasons.append("负债率 N/A")
...
if len(results) > 0:
    ...
    # 保存结果
    output_md = '/Users/john/.openclaw/workspace/buffett_candidates_accurate_real.md'
    output_csv = '/Users/john/.openclaw/workspace/buffett_candidates_accurate_real.csv'
    
    with open(output_md, 'w', encoding='utf-8') as f:
        f.write("# 🐂 巴菲特风格候选股票清单（准确版）\n\n")
        f.write(f"**分析时间**: {datetime.now().strftime('%Y-%m-%d %H:%M')}\n")
        f.write(f"**数据来源**: Tushare Pro (tinyshare 代理)\n")
        f.write(f"**毛利率计算**: 手动计算 = (total_revenue - total_cogs) / total_revenue × 100\n\n")
        f.write("## 筛选标准\n\n")
        ...
        try:
            ...
            if len(df_fina) >= 5:
                ...
                if len(roe_history) >= 5:
                    ...
                    # 获取毛利率
                    df_income = pro.income(ts_code=ts_code, start_date='20230101', end_date='20241231')
                    if len(df_income) > 0:
                        latest_income = df_income.sort_values('ann_date', ascending=False).iloc[0]
                        revenue = latest_income.get('total_revenue')
                        cogs = latest_income.get('total_cogs')
                        if revenue and cogs and revenue != 0:
                            gross_margin = ((revenue - cogs) / revenue) * 100
                            
                            # 获取负债率
                            latest_fina = df_fina.iloc[0]
                            debt_raw = latest_fina.get('debt_to_assets')
                            debt_to_assets = float(debt_raw) if debt_raw and debt_raw != '' else None
                            
                            if gross_margin > 40 and debt_to_assets and debt_to_assets < 50 and not roe_pass:
                                close_calls.append({
                                    '代码': ts_code,
                                    '名称': name,
                                    'ROE_最新': f"{roe_history[0]:.2f}%",
                                    'ROE_平均': f"{sum(roe_history[:5])/5:.2f}%",
    ...
    if close_calls:
        df_close = pd.DataFrame(close_calls)
        df_close = df_close.sort_values('ROE_最新', ascending=False)
